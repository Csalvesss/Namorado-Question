...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-10">
      ...
      {completed.length > 0 && (
        <section className="card grid grid-cols-3 gap-4 px-6 py-5">
          <Stat label="provas feitas" value={String(completed.length)} />
      ...
      <section>
        ...
        {courses.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {courses.map((c) => (
              <Link key={c.id} to={`/curso/${c.id}`} className="card group relative overflow-hidden p-6 transition hover:shadow-wine">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="text-center">
      <div className="font-serif text-3xl font-semibold text-wine-deep">{value}</div>
